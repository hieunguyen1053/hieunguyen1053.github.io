---
title: 'Theo dõi danh mục đầu tư VCBF với AI qua MCP'
...
date: 2026-01-14
tags: ['mcp', 'claude', 'vcbf', 'automation', 'typescript']
# image: './banner.png'
authors: ['honoré']
draft: false
---
...
Chuyện là mình có đầu tư, mỗi chỗ rót một ít, gần đây thì mấy quỹ mở của VCBF tăng lời như đầu tư vàng. Vấn đề là mình bị cái "tật" hay tò mò, cứ thích vào xem giá chứng chỉ quỹ (NAV/1CCQ) nó nhảy nhót thế nào. Dù mình mua đều, mỗi tháng đều mua, nhưng phải lựa ngày mà giá nó tối ưu nhất để mua cho nó stonk. Mà khổ nỗi, mỗi lần muốn check là lại phải:

1. Mở tab mới.
2. Google mò vào trang VCBF.
3. Ngồi chờ web load (nhiều lúc mạng mẽo chán không tả được).
4. Nheo mắt soi cái biểu đồ hoặc bảng giá để tìm ngày mới nhất.
...
Mình dùng `axios` để kéo source HTML về, rồi dùng `cheerio` để bóc tách cái cục JSON đó ra. Code nó "cục súc" kiểu này đây:

```typescript
// src/vcbf.ts
export async function fetchFundData(symbol: string): Promise<FundData[]> {
  const url = FUND_URLS[symbol];
  const response = await axios.get(url); // Kéo HTML về
  const $ = cheerio.load(response.data); // Load vào Cheerio để parse
  let jsonData: FundData[] | null = null;

  // Lục lọi các thẻ script để tìm kho báu
  $('script').each((i, el) => {
    const scriptContent = $(el).html();
    // Dấu hiệu nhận biết data nằm ở đây
    if (scriptContent?.includes('var json_parse = JSON.parse')) {
        // Dùng chút ma thuật Regex để gắp chuỗi JSON ra
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    // Kiểm tra trong túi xem có hàng chưa
    let data = cache.get(symbol);
    if (!data) {
        // Chưa có thì mới đi lấy
        data = await fetchFundData(symbol);
        cache.set(symbol, data);
    }
    
    // Đoạn này là logic lọc thời gian (1D, 1Y...) để Claude đỡ bị ngợp data
    // ...
...
## Kết quả: Phê chữ ê kéo dài!

Cấu hình xong xuôi vào app Claude Desktop, giờ chat sướng hẳn:

**Mình:** "Giá VCBF-MGF hôm nay thế nào so với tuần trước em?"
**Claude:** *(Âm thầm chạy tool...)* "Dạ anh, dữ liệu ngày 12/01/2026 thì giá VCBF-MGF đang là 10,000 VND. So với tuần trước thì..."
